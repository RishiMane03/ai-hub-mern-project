...
function Code() {
  useEffect(()=>{
    ...
    useEffect(() => {
      if (yourCode.content) {
        const typingEffectInterval = setInterval(() => {
          if (yourCode.content.length > typingContent.length) {
            setTypingContent((prevContent) => yourCode.content.slice(0, prevContent.length + 1));
          } else {
    ...
    }, [yourCode.content, typingContent]);

    async function handleCode(){
        setTypingContent('')
        setyourCode('')
        if(!inputedCode){
          toast.error(`${userInfo} Please enter your question`)
          return
        }
        try {
            setLoaderDisplay(true)
            const response = await axios.post(`${window.location.origin}/getCode`, { inputedCode,language });
    ...
    function copyToClipboard() {
      if (yourCode.content) {
        navigator.clipboard.writeText(yourCode.content);
