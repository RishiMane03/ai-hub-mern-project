...
const RightSection = () => {
  ...
  const logoutFun = async () => {
    try {
      ...
      if(!response.data.isTokenPresent){
    ...
    async function handleSaveChat(){
        try {
            const userIdToken = getCookie('accessToken');
            if(!userIdToken){
              navigate('/login')
              toast.error(`${userInfo} Token expire, Please login!`)
            }
            ...
            if(allMessages.length > 0){
              ...
              toast.error(`${userInfo} There is nothing to create memories!`)
            }
        } catch (error) {
            if(allMessages.length > 0){
              toast.success('Memories created!')
                ...
                allMessages.length > 0 ?
                        ...
                        {allMessages.map((msg, index) => (
                            <div key={index} className='message'>
                                ...
                                <div className='details'>
                                    <h2 style={{marginTop: '0.5rem'}}>{msg.role === 'user' ? (userInfo ? `${userInfo}` : 'You') : ('AI')}</h2>
                    ...
                    <div className='nochat'>
                        <div className='s1'>
                            ...
                            <h1>{userInfo} how can I help you today?</h1>
                        </div>
                    </div>
            }
