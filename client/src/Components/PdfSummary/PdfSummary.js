...
function PdfSummary() {  
  ...
  async function getShortSummary(){
    let myCookieValue = getCookie('accessToken');
        
    if(!myCookieValue){
      toast.error(`${userInfo} Token expire, Please login!`)
    ...
    toast.success('Wait for few seconds')
      if(pdfContent && selectedPdf){
          try {
              const response = await axios.post(`${window.location.origin}/pdfSummary`, { pdfContent });
  ...
  async function getQuestions(){
    ...
    toast.success(`${userInfo} Wait for few seconds`)
    setPdfQuestions('')
      if(pdfContent && selectedPdf){
          try {
              const response = await axios.post(`${window.location.origin}/pdfQuestions`, { pdfContent });
  ...
  const handleEnter = async (e) => {
    if (e.key === 'Enter') {
      ...
      if(question){
        try {
          const response = await axios.post(`${window.location.origin}/askDoubt`, { pdfContent , question });
          
          console.log(response.data.ansToDoubt.content);
          setAnswer(response.data.ansToDoubt.content);
          toast.success(`Your wish is my command ${userInfo}`)
        } catch (error) {
          console.error('Error:', error);
        }
      }else{
      ...
      <div className='aiHeading'>
        ...
        <div className='uploadSection'>
          <h2
            ...
            onDrop={handleDrop}
          >
            {showButtons ? `${file.name} ` : <div> <i class="fa-solid fa-upload fa-bounce fa-l"></i> Hey {userInfo}, Upload or Drag PDF </div>}
