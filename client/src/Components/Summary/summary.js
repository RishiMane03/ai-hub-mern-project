...
function Summary() {
  ...
  const handleSummarize = async () => {
    ...
    if(paragraph === '' || paragraph === null){
      toast.error(`${userInfo} Please add the paragraph`)
      return
    }

    if(paragraph.length < 100){
      toast.error(`${userInfo} Please add at least 100 characters`)
      return
    }

    try {
      setLoaderDisplay(true)
      const response = await axios.post(`${window.location.origin}/summarize`, { paragraph });
  ...
  useEffect(() => {
    if (summary.content) {
      const typingEffectInterval = setInterval(() => {
        if (summary.content.length > typingContent.length) {
          setTypingContent((prevContent) => summary.content.slice(0, prevContent.length + 1));
        } else {
